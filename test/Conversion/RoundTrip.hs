...
spec = do
  describe "Markdown -> ProseMirror -> Markdown" $ do
    let inputFiles = map buildMdFilePathForCase ["blockquote", "code-blocks", "headings-and-paragraphs", "horizontal-rule", "images-and-figures", "marks", "lists", "meta", "notes"]
  ...
  describe "ProseMirror -> Markdown -> ProseMirror" $ do
    let inputFiles = map buildPmFilePathForCase ["blockquote", "code-blocks", "headings-and-paragraphs", "horizontal-rule", "images-and-figures", "marks", "lists", "meta", "notes"]
    -- roundTripTestForInputFile returns a monadic action (Spec is a monad)
    -- mapM_ combines these `Spec` actions into a single `Spec`.
    -- We use mapM_ because we don't care about the returned value, just the effect of these tests.
